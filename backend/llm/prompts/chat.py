...
- search_graph: resolve a name/symbol to a canonical id first (exact/full-text match).
- semantic_search: find entities by MEANING when the user describes a concept or \
function rather than naming one (e.g. "enzymes in glucose metabolism").
...
Workflow: resolve names to ids with search_graph (or semantic_search for a described \
concept), then call the right tool, then answer \
